defmodule PINXS.Customers.Customer do
  ...
  """
  def add_card(%Customer{token: token}, %Card{} = card, config) do
    API.post("/customers/#{token}/cards", card, Card, config)
  end

  def add_card(%Customer{token: token}, card_token, config)
  ...
  Creates a customer
  """
  def create(%Customer{card: card} = customer, config) when not is_nil(card),
    do: create_customer(customer, config)

  def create(%Customer{card_token: card_token} = customer, config)
  ...
  Deletes a customer
  """
  def delete(%Customer{token: token}, config) do
  ...
  """
  def delete_card(%Customer{token: token}, card_token, config) do
  ...
  """
  def get_cards(%Customer{token: token}, config) do
  ...
  """
  def get_charges(%Customer{token: token}, config) do
  ...
  Updates a customer
  """
  def update(%Customer{token: token}, params, config) when not is_nil(token) do
